...
class SubmissionAgent(BaseAgent):
    ...
    async def run_task(self, task_id: str, payload: dict) -> None:
        ...
        async with stdio_client(StdioServerParameters(command="python", args=[ATS_SERVER])) as (ra, wa):
            async with ClientSession(ra, wa) as ats:
                await ats.initialize()
                self._ats_session = ats

                async with stdio_client(StdioServerParameters(command="python", args=[VMS_SERVER])) as (rv, wv):
                    async with ClientSession(rv, wv) as vms:
                        await vms.initialize()
                        self._vms_session = vms

                        async with stdio_client(StdioServerParameters(command="python", args=[COMM_SERVER])) as (rc, wc):
