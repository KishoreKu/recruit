...
class BaseAgent(ABC):
    ...
    async def _call_tool(self, session: ClientSession, tool: str, **kwargs) -> dict:
        """Call an MCP tool and return its result dict."""
    ...
    async def fail_task(self, task_id: str, error: str, attempts: int, max_attempts: int) -> None:
        ...
        async with pool.acquire() as conn:
            if attempts >= max_attempts:
                ...
                new_status = "pending"
            await conn.execute(
                "UPDATE agent_task_queue SET status=$1, last_error=$2, updated_at=NOW() WHERE id=$3",
            ...
            try:
                ...
                if task:
                    ...
                    try:
                        ...
                        await self.fail_task(task["id"], str(exc), task["attempts"], task["max_attempts"])
                        await self._log_health("failed", str(exc))
                else:
