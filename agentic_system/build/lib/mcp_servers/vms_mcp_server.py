...
@mcp.tool()
async def fetch_new_requisitions(limit: int = 20) -> list[dict]:
    """
    Fetch open requisitions from the VMS database that haven't been matched yet.
    In production, this also polls external VMS APIs.
    """
    pool = await get_pool()
    ...
    logger.info(f"[VMS] Fetched {len(results)} open requisitions.")
    return results
