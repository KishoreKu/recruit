...
class SelfHealingAgent:
    ...
    async def alert_human_review_tasks(self) -> int:
        """
        Find NEW tasks requiring human review (not yet alerted) and send ONE
        admin email. Marks alerted tasks so they are never re-emailed.
        Uses the 'assigned_agent' column as an 'alerted' flag to avoid a
        schema migration (set to '__alerted__' after emailing).
        """
        pool = await get_pool()
        async with pool.acquire() as conn:
            # Only fetch tasks we haven't already alerted on
        ...
        # Mark them all as alerted immediately so we never email again
        task_ids = [r['id'] for r in rows]
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE agent_task_queue SET assigned_agent = '__alerted__' WHERE id = ANY($1)",
                task_ids,
            )
        ...
        logger.warning(f"[Self-Healing] Sending ONE alert for {len(rows)} new human_review task(s).")
        await self._log_health("circuit_open", f"Alerting on {len(rows)} new human_review tasks.", {"count": len(rows)})

        # Send a single admin notification via MS Graph (if configured)
        if settings.MS_TENANT_ID and settings.MS_CLIENT_ID:
            try:
                ...
                <p><em>This is a one-time alert. You will only be emailed again if new tasks enter human_review.</em></p>
                """
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    if token:
                        ...
                        mail_payload = {
                            "message": {
                                "subject": f"[Westley Agents] {len(rows)} task(s) need human review (one-time alert)",
                        ...
                        logger.info("[Self-Healing] Human review alert email sent.")
