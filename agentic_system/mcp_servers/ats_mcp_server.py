...
import os
logger.info(f"[ATS MCP] OS env keys: {list(os.environ.keys())}")
logger.info(f"[ATS MCP] AZURE_OPENAI_KEY in env: {bool(os.environ.get('AZURE_OPENAI_KEY'))}")
logger.info(f"[ATS MCP] Settings AZURE_OPENAI_KEY: {bool(settings.AZURE_OPENAI_KEY)}")
logger.info(f"[ATS MCP] Settings AZURE_OPENAI_ENDPOINT: {settings.AZURE_OPENAI_ENDPOINT}")
